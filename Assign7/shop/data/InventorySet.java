...
final class InventorySet implements Inventory {
	...
	Record checkOut(Video video) {
		...
		RecordObj rObj = new RecordObj(video, record.numOwned(), record.numOut() + 1, record.numRentals() + 1);
		_data.put(video, rObj);
		return record;
	}
	...
	Record checkIn(Video video) {
		...
		if (record == null || record.numOut() == 0)
			throw new IllegalArgumentException();
		RecordObj rObj = new RecordObj(video, record.numOwned(), record.numOut() - 1, record.numRentals());
		_data.put(video, rObj);
		return record;
	}
	 ...
	 */
	Map clear() {
		Map<Video, Record> recordValues = _data;
		_data = new HashMap<Video, Record>();
		return recordValues;
	}
